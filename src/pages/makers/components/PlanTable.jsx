                    ...
                    <Table.Row key={`${d.foodName + di}`}>
                      ...
                      {di === 0 && si === 0 && (
                        <Table.Cell padding="0px" rowSpan={count[i]}>
                          ...
                          <Button
                          toggle
                          color={ v.scheduleStatus === 0? "grey":v.scheduleStatus===1 ? "green":"red" }
                        ...
                        // }))}}
                        >
                          {v.scheduleStatus === 0? '대기':v.scheduleStatus===1 ? "승인":"거절"}
                        </Button>
                          </FlexBox>
                        </Table.Cell>
                      )}
                      ...
                      <Table.Cell textAlign="center">
                        <Button
                          toggle
                          color={ d.scheduleStatus === 0? "grey":d.scheduleStatus===1 ? "green":"red" }
                        //   onClick={()=>{setTestData(testData.map((makers)=>{
                        //     return {...makers ,clientSchedule:makers.clientSchedule.map((client)=>{
                        //       return {...client,foodSchedule:client.foodSchedule.map((food)=>{
                        //         if(food.presetFoodId === d.presetFoodId){
                        ...
                        // }))}}
                        >
                          {d.scheduleStatus === 0? '대기':d.scheduleStatus===1 ? "승인":"거절"}
                        </Button>
                      </Table.Cell>
